...
const Womenseye = () => {
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        // Log the data to check if it is structured correctly
        console.log("Fetched data for Womenssun:", data);
        ...
        const groupedProducts = data.reduce((acc, item) => {
          ...
          if (item.color) { // Consistent property name
  ...
  return (
    <>
      <Header />
      <div className="mens-benner" style={{ backgroundImage: 'url(../files/banner_img/A7_02553.jpeg)' }}>
        ...
        <div className="row row-cols-1 p-1 g-2">
          {products.map((product) => (
            <div id="pdc" className="col-4" key={product.model_number}>
              <div className="card">
                <Link className='tpimg' to={`/women/${product.model_number}`} style={{ textDecoration: 'none' }}>
                  <img
                    src={hoveredImage[product.model_number] || `https://13-century.com/files/women/${product.model_number}/${product.variants[0]?.color}.png`}
                    ...
                    {product.variants.map((variant) => (
                      <div
                        ...
                        onMouseEnter={() => setHoveredImage((prev) => ({
                          ...prev,
                          [product.model_number]: `https://13-century.com/files/women/${product.model_number}/${variant.color}.png`
                        }))}
                        onMouseLeave={() => setHoveredImage((prev) => ({
                          ...prev,
                          [product.model_number]: `https://13-century.com/files/women/${product.model_number}/${product.variants[0]?.color}.png`
                        }))}
                      >
                        <img
                          src={`https://13-century.com/files/women/${product.model_number}/${variant.color}.png`}
                          alt={variant.color}
                          loading='lazy'
                        />
                      </div>
